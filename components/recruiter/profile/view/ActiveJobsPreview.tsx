      ...
      {isLoading ? (
        ...
        <div className="flex gap-4 overflow-x-auto pb-4 lg:grid lg:grid-cols-3 lg:overflow-visible">
          {activeJobs.map((job) => (
            <article
              key={job.id}
              className="min-w-70 rounded-xl border border-slate/10 bg-white p-5 shadow-sm transition-all duration-300 hover:shadow-lg lg:min-w-0"
            >
              ...
              <h3 className="mb-1 font-display text-base font-semibold text-midnight">
                {job.title}
              </h3>
              ...
              <div className="flex items-center justify-between border-t border-slate/10 pt-4">
                <span className="text-xs text-slate">
                  {formatRelativeDays(job.createdAt)}
                </span>
                <button
                  className="p-2 text-slate transition-colors hover:rounded-lg hover:bg-primary/5 hover:text-primary"
                  aria-label="Save job"
                >
